...
VTM3DMap.Mode="Viewer";
VTM3DMap = VTM3DMap.InitX3D('VTM3DMap');
window.onresize=function(e){
	VTM3DMap.OnResize(e);
};
VTM3DMap.MouseMoveCallBack=function(e,n){
	$('#ShowNodeName').html('');
	
	$('#ShowShapeName').html('');
	if(n.attr('_id')){		
	   var Node = VTM3DMap.ShapeSelectBox[n.attr('_id')];
	   $('#ShowShapeName').html(Node.name);
	};
	
	var Node = $(VTM3DMap.CurrentShapeSelected.hitObject).parent();
	if(Node.attr('node_id')){
	  $('#ShowNodeName').html(VTM3DMap.NodesSelectBox[Node.attr('node_id')].name);
	};
};
VTM3DMap.MouseMoveNodeCallBack=function(e,n){
...
Res.VTM3DMap.AfterInsertX3DElement=function(P){
	if(P.CurrentNodesSelectBox!=''){
	   $(P.NodesSelectBoxPoint).find('option[value="'+P.CurrentNodesSelectBox['_id']+'"]').remove();
	};
...
Res.VTM3DMap.AfterDeleteElement=function(P){
