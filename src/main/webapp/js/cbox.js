...
(function (window, rootobj) {
    ...
    var applyCssGradient = function (o, colors) {
        ...
        try {
            o.style.backgroundImage = "linear-gradient(" + colorstrings.join(",") + ")";
            bi = o.style.backgroundImage;
            if (bi !== "none" && bi !== "")return true;
        } catch (ex) {
        }
        try {
            o.style.backgroundImage = "-moz-linear-gradient(" + colorstrings.join(",") + ")";
            bi = o.style.backgroundImage;
            if (bi !== "none" && bi !== "")return true;
        } catch (ex) {
        }
        ...
        try {
            o.style.background = "-webkit-gradient(" + gradientstring + ")";
            bi = o.style.backgroundImage;
            if (bi !== "none" && bi !== "")return true;
        } catch (ex) {
        }
        try {
            o.style.backgroundImage = "-o-linear-gradient(" + colorstrings.join(",") + ")";
            bi = o.style.backgroundImage;
            if (bi !== "none" && bi !== "")return true;
        } catch (ex) {
        }
        try {
            o.style.backgroundImage = "-ms-linear-gradient(" + colorstrings.join(",") + ")";
            bi = o.style.backgroundImage;
            if (bi !== "none" && bi !== "")return true;
        } catch (ex) {
        }
        if (colors.length === 2 && "filters" in o) {
            ...
            var tohex = function (v) {
                ...
                var d = 0;
                var c = ((d = Math.round(v)) < 0 ? 0 : (d > 255 ? 255 : d));
    ...
    var ColorSpace = subclass(Object, {
        ...
        getcolor: function (x, y, current) { // for display only
            ...
            if (area === 1) {
                ...
                rgba[3] = ((current[3] === null || typeof current[3] === "undefined")) ? 255 : current[3];
                return rgba;
            } else if (area === 2) {
...
////////////////
    ...
    var namedColorsDatalist = function () {
        ...
        var dlid = 0;
        do {
            datalistid = "datalist-colorpicker" + dlid;
            dlid += 1;
...
// is deprecated
    ...
    var supportsPattern = function () {
        ...
        if (!("pattern" in inp) || typeof inp.validity === "undefined") {
            _supportsPattern = false;
            return _supportsPattern;
        }
    ...
    var validatePattern = function (e) {
        ...
        for (var i = 0; i < e.target.elements.length; i++) {
            ...
            if (inp.getAttribute("required") !== null && inp.value.length === 0) {
                window.alert("Please fill in the field: " + inp.title);
                inval = true;
            } else if (inp.value.length > 0 && !(new RegExp("^(?:" + a + ")$").test(inp.value))) {
                window.alert("Please match the requested format: " + inp.title);
                inval = true;
            }
            if (inval) {
                inp.focus();
    ...
    var setPatternAndTitle = function (thisInput, usealpha) {
        ...
        if (thisInput.tagName.toLowerCase() === "input" && thisInput.type === "text") {
            ...
            if (window.opera && supportsPattern()) {
                ...
                addListener(thisInput, "focus", function () {
                    this.setAttribute("pattern", patternstart + patternother);
                });
                addListener(thisInput, "blur", function () {
...
////////////////
    var MyColorPicker = subclass(Object, {
        initialize: function (info, parent, startingvalue, usealpha) {
            ...
            try {
                this.p.style.borderRadius = this.padding + "px";
            } catch (ex) {
            }
        ...
        updatedivs: function (area) {
            ...
            if (area === 1)areafunc = this.isInAreas1;
                ...
                justswatch = true;
                areafunc = this.isInAreas2;
            }
            else if ((area !== null && typeof area !== "undefined")) {
                justswatch = true;
                areafunc = this.isInAreas3;
            }
    ...
    var EventHandlers = subclass(Object, {
        ...
        add: function (func) {
            if (func)this.handlers.push(func);
        },
    ...
    var dobgcolordelayfunc = function (o, val) {
        return function () {
            if (o.getAttribute("data-currentbgcolor") === val) {
                try {
                    o.style.background = val;
                } catch (ex) {
                    o.style.background = rgbToColorHtml(colorToRgba(val));
                }
    ...
    rootobj.createColorPickerButton = function (thisInput, extra) {
        ...
        coloredInput(thisInput, newInput);
        try {
            newInput.style.textShadow = "none";
        } catch (ex) {
        }
        var bid = 0;
        var bidstring = "";
        do {
            bidstring = "colorpickerbuttonid" + bid;
            bid += 1;
    ...
    addReadyListener(function () { // set up color pickers
        ...
        for (i = 0; i < inputs.length; i++) {
            inputsArray[inputsArray.length] = inputs[i];
        }
        for (i = 0; i < inputsArray.length; i++) {
            var thisInput = inputsArray[i];
            if (thisInput.getAttribute("type") === "color" ||
                (thisInput.type === "text" && rootobj.hasIdOrClassName(thisInput, "frm:color_"))) {
