...
function refreshDatePicker(dateFieldName, year, month, day) {
    ...
    var xTD = "</td>" + crlf;
    var DIV_title = "<div class='dpTitleText'  style='width:91px' align='right'>";
    var DIV_selected = "<div class='dpDayHighlight'>";
    var xDIV = "</div>";
    var TD_input = "<input type='text' maxlength='4' style='width: 32px' id='year' value='" + thisDay[0] + "'";
    TD_input += " onBlur='updateYear(\"" + dateFieldName + "\");'/>";
    ...
    html += TD_title + DIV_title + monthArrayLong[ thisDay[1] - 1 ] + TD_input + xDIV + xTD;
    html += TD_buttons + getButtonCode(dateFieldName, thisDay, +1, "&gt;") + xTD;// > //
    html += TD_buttons + getButtonCodeYear(dateFieldName, thisDay, +1, "&gt;&gt;") + xTD;// >> //
    html += xTR;
    html += "</table></td></tr>"
...
function getButtonCodeYear(dateFieldName, dateVal, adjust, label) {
    ...
    var newYear = (parseInt(dateVal[0]) + adjust);
    return "<button class='dpButton' onClick='refreshDatePicker(\"" + dateFieldName + "\", "
        + newYear + ", " + newMonth + ");'>" + label + "</button>";
}
...
function updateYear(dateFieldName, dateVal) {
    var newYear = parseInt(document.getElementById('year').value);
    return  refreshDatePicker(dateFieldName, newYear, 1, 0);
}
