...
// ---- Common Options ----
var gMonths = ["فروردين", "ارديبهشت", "خرداد", "تير", "مرداد", "شهريور", "مهر", "آبان", "آذر", "دي", "بهمن", "اسفند"];
var gWeekDay = ["ش", "ي", "د", "س", "چ", "پ", "ج"];	// weekday caption from Sunday to Saturday

var gBegin = [1278, 10, 11];	// calendar date range begin from [Year,Month,Date]. Using gToday here will make it start from today.
var gEnd = [1428, 10, 10];	// calendar date range end at [Year,Month,Date]
var gsOutOfRange = "تاریخ نامعتبر است!";	// out-of-date-range error message. If set to "", no alerts will popup on such error.
...
var gsNavPrev = "<IMG id='navPrev' class='MonthNav' src='images/arrowr.gif' width='18' height='18' border='0' alt='' onmousedown='showPrevMon()' onmouseup='stopShowMon()' onmouseout='stopShowMon();if(this.blur)this.blur()'>";	// the content of the left month navigator
var gsNavNext = "<IMG id='navNext' class='MonthNav' src='images/arrowl.gif' width='18' height='18' border='0' alt='' onmousedown='showNextMon()' onmouseup='stopShowMon()' onmouseout='stopShowMon();if(this.blur)this.blur()'>";	// the content of the right month navigator

var gbHideBottom = false;	// true: hide the bottom section; false: show it with gsBottom.
var gsBottom = (NN4 ? "" : "<DIV class='BottomDiv'>") + "<A class='BottomAnchor' href='javascript:void(0)' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"You cannot select today!\");return false;' onmouseover='return true;' >امروز</A>" + (NN4 ? "" : "</DIV>");	// the content of the bottom section.
...
var gsAction = " ";	// default adapter to be eval()'ed on everyday except the days with agendas, which have their own actions defined in agendas.
...
var gcFri = "red";	// Firday font color
var gcFriBG = null;	// Friday background color
...
var gAgendaMask = [-1, -1, gcCellBG, "black", -1, false, null];	// [message, adapter, bgcolor, fgcolor, bgimg, boxit, html] - Set the relevant bit to -1 to keep the original agenda/event value of that bit intact. Any other value will be used to override the original one defined in agenda.js. Check the tutorial for details.
